...
>
> 문서 버전: v1.0 (2026-06-11) · 상태: P4 구현 기준
...
- 독립 가정의 일제사 요격률 1−(1−p₁)^N은 **상한**이다: 조준 바이어스(트랙 오차)와 거스트는 전탄 공통모드라 탄착이 상관되고, 실측은 이보다 낮다 — 실험 보고서 §6의 분석 대상.
...
| §4 RK4 외탄도 | 해석해·수렴·교차언어 | ballistics_test: VacuumParabola·HalfStep·NumpyReference |
| §5 Pd 곡선 | 단조·기준점·우천 | radar_test: PdMonotonically·PdIsHalf·RainAttenuation |
| §5 호라이즌 | 시스키머 늦은 탐지 | radar_test: HorizonMasks·SeaSkimmerDetectedLater |
...
| §8 PIP 고정점 | 수렴·입력 무관성 | fire_control_test 전체 + SolverConvergence (P2) |
