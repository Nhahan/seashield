...
>
> 문서 버전: v1.0 (2026-06-11) · 상태: P3 구현 기준
...
| type | 메시지 | 필드 | 비고 |
|---|---|---|---|
| 1 | ClientHello | version u16, role u8, token u64 | token 0 = 신규, ≠0 = 재접속 |
| 2 | ServerWelcome | token u64, role u8, udp_port u16, tick_rate u16, snapshot_rate u16, weather str16 | weather는 표시용 요약(≤512B) |
| 3 | ServerReject | reason u8 | 0 버전 불일치 / 1 역할 점유 / 2 만석 / 3 무효 토큰 |
| 4 | FireRequest | az f64, el f64, salvo u16, dispersion f64, interval f64 | 서버가 범위 재검증 후 sim에 투입 |
...
| 20 | EngagementEvent | **Reliable** | tick u32, kind u8, rocket_id u16, miss f32, detonated u8, killed u8 |

EventKind: 0 발사 / 1 탄 판정(요격/실패) / 2 표적 격파 / 3 교전 종료. 이벤트가 자체 tick을 가지므로 수신측이 순서를 재구성한다(Reliable-Unordered로 충분한 근거, 기획서 §5.7).
...
| 필드 | 크기 | 양자화 |
|---|---|---|
| id | u16 | 로켓 id 하위 16비트 (단일 교전 규모에서 충돌 없음) |
| kind | u8 | 0 표적 / 1 로켓탄 |
| state | u8 | 표적: 생존/격파. 로켓: 부스트/활공 |
| flags | u8 | 예약 |
...
- 클라이언트 보간 버퍼(기획서 §4.7)는 표현 계층 — P5 UE5 클라이언트 소관. 더미 클라이언트는 수신 통계만 검증한다.
