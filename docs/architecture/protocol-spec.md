...
>
> 문서 버전: v1.4 (2026-06-14) · 상태: P7+ 구현 기준 (프로토콜 버전 5)
...
> v1.4: 프로토콜 v5 — 플레이어 조타 함선: kOwnShip 엔티티(스냅샷에 자함 자세) + ShipCommand 제어 메시지(rudder/throttle, 클라→서버 TCP). 변경 이력은 §11.
...
| 6 | ShipCommand | rudder f64, throttle f64 | **자함 조타**(v5). rudder∈[-1,1](+우현)·throttle∈[0,1] 보유 set-point — 변경 시에만 송신(보유값 유지)이라 스냅샷 주기가 아닌 reliable TCP로 탄다. 서버가 범위 클램프 후 sim 스레드로 전달, 단일교전 경로는 저널에 `steer` 줄로 기록(리플레이 재현). Weapons/Solo만 |
...
| kind | u8 | 0 표적 / 1 로켓탄 / **2 트랙(칼만 추정 — 콘솔이 실제로 그리는 것, P4)** / **3 자함(kOwnShip, 자세 — P7+, 델타 코덱 kind 2비트의 마지막 슬롯)** |
...
| 5 | P7+ | kOwnShip 엔티티 3(자함 자세 — 델타 코덱 kind 2비트의 마지막 슬롯), ShipCommand 6(조타 set-point, TCP). 범프 근거 동일(strict 디코더 + 단일 레포). 자함은 기본 고정 플랫폼이라 단일교전/골든은 자세 필드만 추가(결정론 해시 1회 재생성) |
