...
>
> 문서 버전: v1.1 (2026-06-11) · 상태: P4 구현 기준 (프로토콜 버전 2)
> 구현: `protocol/` (서버·더미 클라이언트·UE5 클라이언트가 공유하는 독립 라이브러리 — 표준 라이브러리 외 의존성 0)
> v1.1: 프로토콜 v2 — kTrack 엔티티(추정 트랙 스트림), 트랙 수명주기 이벤트, FireRequest 트랙 지정, FireSolution 정의, 서버 리플레이 모드. 변경 이력은 §11.
...
| 21 | FireSolution | Unreliable | tick u32, track_id u16, valid u8, pip 3×i24(1cm), tof f32, dispersion_radius f32 — **v2에서 정의·테스트 완료, 서버 주기 송신은 P5**(무기통제 UI와 함께; §4.3의 "PIP 갱신 UDP" 약속을 버전 범프 없이 이행하기 위한 선정의) |
