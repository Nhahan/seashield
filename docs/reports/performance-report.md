...
### 6.21 워터 from-scratch 재구축 + 회귀수정 · 멀티클라 증명 · AAR 타임라인 (2026-06-19)

§6.19/6.20의 Path B(`M_SeaOceanWPO`/`ocean_grid.py`)는 폐기되고, **from-scratch 오션**(`setup_materials.make_ocean` + `tools/assets/ocean.py` + `apply_ocean.py`)으로 대체됨 — 커스텀 dense grid `SM_Ocean` + 32-wave Gerstner WPO + Substrate Default-Lit BLEND_TRANSLUCENT surface-forward(Beer-Lambert 깊이색·Jacobian 화이트캡·Fresnel sky-floor·분석적 sun-glitter·SSS). 렌더 전용(부력은 hidden 플러그인 오션 샘플) → **결정론/골든 무영향**(Debug+Release ctest **224/224 PASS** 재확인). fps 무게이트(owner: 최고 품질).
